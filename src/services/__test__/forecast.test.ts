...
import { Ordination } from '@src/util/orderByCustomProp';
...
describe('Forecast Service', () => {
  ...
  it('should return the forecast for mutiple beaches in the same hour with different ratings ordered by windSpeed in desc', async () => {
    mockedStormGlassService.fetchPoints.mockResolvedValueOnce([
      {
        swellDirection: 123.41,
        swellHeight: 0.21,
    ...
    const beachesWithRating = await forecast.processForecastForBeaches(
      beaches,
      { customProp: 'windSpeed', ordination: Ordination.DESC }
    );
    expect(beachesWithRating).toEqual(expectedResponse);
  });

  it('should return the forecast for mutiple beaches in the same hour with different ratings ordered by swellPeriod in asc', async () => {
    mockedStormGlassService.fetchPoints.mockResolvedValueOnce([
      {
        swellDirection: 123.41,
        swellHeight: 0.21,
    ...
    const beachesWithRating = await forecast.processForecastForBeaches(
      beaches,
      { customProp: 'swellPeriod', ordination: Ordination.ASC }
    );
    expect(beachesWithRating).toEqual(expectedResponse);
  });
